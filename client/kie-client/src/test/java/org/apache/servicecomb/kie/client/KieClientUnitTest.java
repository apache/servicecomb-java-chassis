 ...
 */
public class KieClientUnitTest {
  ...
  public void putKeyValue() throws IOException {
    ...
    Mockito.when(kieRawClient.putHttpRequest("/kie/kv/test1", null, JSON.toJSONString(kvBody)))
    ...
    Assert.assertEquals("test1", JSONObject.parseObject(kvResponses).getString("key"));
    Assert.assertEquals("test", JSONObject.parseObject(kvResponses).getString("value"));
  }
