...
public class KieClient {
   ...
   * @return
   */
  public String putKeyValue(String key, KVBody kyBody) {
    try {
      HttpResponse response = httpClient.putHttpRequest("/kie/kv/" + key, null, JSON.toJSONString(kyBody));
  ...
  public List<KVResponse> getValueOfKey(String key) {
    try {
      ...
      if (response.getStatusCode() == HttpStatus.SC_OK) {
        ...
        return JSONObject.parseArray(response.getContent(), KVResponse.class);
      } else {
        throw new OperationsException(response.getStatusCode() + response.getMessage() + response.getContent());
      }
    } catch (IOException | OperationsException e) {
