...
public class RefreshableServiceRegistryCacheTest {
  ...
  @Test
  public void clear_cache_revision() {
    ...
    microserviceCache2.revisionId = "rev";

    serviceRegistryCache.clearRevisionId(
        MicroserviceCacheKey.builder().serviceName("svc").env("env").appId("appId").build());

    Assert.assertNull(microserviceCache.revisionId);
    Assert.assertEquals("rev", microserviceCache2.revisionId);

    microserviceCache.revisionId = "rev";

    serviceRegistryCache.clearAllRevisionId();
    Assert.assertNull(microserviceCache.revisionId);
    Assert.assertNull(microserviceCache2.revisionId);
  }
